...
void readParameters(ros::NodeHandle pnh){
	...
	pnh.param("map_server_name", map_server_name, string("/map_server"));
	pnh.param("map_topic", map_topic, string("/map"));
}
...
void constructCommands(){ 
	...
	killMapServerCommand = "rosnode kill " + map_server_name;
	startMapServerInBackgroundCommand = "rosrun map_server map_server " + map_file_path + "/indoor.yaml __name:=" + map_server_name + " /map:=" + map_topic + " &";
